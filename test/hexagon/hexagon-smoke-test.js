...
var request = require('supertest'),
    ...
    modulePath = "../../dist/connection-grid";
...
describe('Hexagon smoke test', function() {
    ...
    it('Hexagon method with no spec should return object', function(done) {
        var grid = _module.Hexagon();
        should.exist(grid);
        done();
    });
    ...
    it('xSize should return size of x dimension', function(done) {
        let sizeX = 5;
        let sizeY = 6;
        var grid = _module.Hexagon({ x: sizeX, y: sizeY });
    ...
    it('set method with valid parameter should return true', function(done) {
        var grid = _module.Hexagon({ x: 1, y: 1 });
        should.exist(grid);
   ...
   it('cloneArray method should return a clone of the internal array', function(done) {
    ...
    it('getNeighborDirs should return correct lists', function(done) {
        var grid = _module.Hexagon({ x: 5, y: 5 });
        should.exist(grid);
        grid.getNeighborDirs(1,1).should.eql([ "N", "S", "E", "W", "SW", "SE" ]);
        grid.getNeighborDirs(2,1).should.eql([ "N", "S", "E", "W", "NW", "NE" ]);
        done();
    });
    ...
    it('markVisited should return true for valid cell', function(done) {
        var grid = _module.Hexagon({ x: 3, y: 3 });
        should.exist(grid);
        let tX = 0;
        let tY = 0;
        let VISITED = 0x01;
